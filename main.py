...
def manager_personale_create(
    ...
    data_assunzione: str | None = Form(None),
    ...
    try:
        nuovo_personale = Personale(
            ...
            data_assunzione=parse_date(data_assunzione),
...
def manager_personale_edit(
    ...
    db = SessionLocal()
    try:
        personale_obj = db.query(Personale).filter(Personale.id == personale_id).first()
    finally:
        db.close()
...
def manager_personale_update(
    ...
    try:
        ...
        personale_obj.data_assunzione = parse_date(data_assunzione)
        personale_obj.attivo = attivo is not None
...
def manager_veicoli_list(
    ...
    try:
        veicoli_list = (
            db.query(Veicolo)
            .options(
                joinedload(Veicolo.personale),
                joinedload(Veicolo.cantiere),
            )
            .order_by(Veicolo.targa.asc())
            .all()
        )
    finally:
        db.close()
...
def manager_veicoli_new(
    ...
    try:
        ...
        cantieri_list = db.query(Site).order_by(Site.name.asc()).all()
    finally:
        db.close()

    return templates.TemplateResponse(
        "manager/veicoli_form.html",
        {
            "request": request,
            "user": current_user,
            "veicolo": None,
            "personale": personale_list,
            "cantieri": cantieri_list,
        },
    )
...
def manager_veicoli_create(
    ...
    modello: str | None = Form(None),
    anno: str | None = Form(None),
    tipo: str | None = Form(None),
    km_attuali: str | None = Form(None),
    scadenza_revisione: str | None = Form(None),
    scadenza_assicurazione: str | None = Form(None),
    personale_id: str | None = Form(None),
    cantiere_id: str | None = Form(None),
    note: str | None = Form(None),
    current_user: User = Depends(get_current_active_user_html),
):
    if current_user.role not in (RoleEnum.admin, RoleEnum.manager):
    ...
    try:
        nuovo_veicolo = Veicolo(
            targa=(targa or "").strip().upper(),
            marca=(marca or "").strip() or None,
            modello=(modello or "").strip() or None,
            anno=parse_int(anno),
            tipo=(tipo or "").strip() or None,
            km_attuali=parse_int(km_attuali),
            scadenza_revisione=parse_date(scadenza_revisione),
            scadenza_assicurazione=parse_date(scadenza_assicurazione),
            personale_id=parse_int(personale_id),
            cantiere_id=parse_int(cantiere_id),
            note=(note or "").strip() or None,
        )
        db.add(nuovo_veicolo)
        db.commit()
    finally:
        db.close()
...
def manager_veicoli_update(
    ...
    db = SessionLocal()
    try:
        veicolo_obj = db.query(Veicolo).filter(Veicolo.id == veicolo_id).first()
        if not veicolo_obj:
            return RedirectResponse(
                url=request.url_for("manager_veicoli_list"), status_code=303
            )

        veicolo_obj.targa = (targa or "").strip().upper()
        veicolo_obj.marca = (marca or "").strip() or None
        veicolo_obj.modello = (modello or "").strip() or None
        veicolo_obj.anno = parse_int(anno)
        veicolo_obj.tipo = (tipo or "").strip() or None
        veicolo_obj.km_attuali = parse_int(km_attuali)
        veicolo_obj.scadenza_revisione = parse_date(scadenza_revisione)
        veicolo_obj.scadenza_assicurazione = parse_date(scadenza_assicurazione)
        veicolo_obj.personale_id = parse_int(personale_id)
        veicolo_obj.cantiere_id = parse_int(cantiere_id)
        veicolo_obj.note = (note or "").strip() or None
