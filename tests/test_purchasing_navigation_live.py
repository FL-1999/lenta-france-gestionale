...
def test_navigation_b_preserves_context_filters_and_role_boundaries(live_operations):
    ...
    with sync_playwright() as pw:
        ...
        expect(page.locator('.purchase-tabs [data-purchase-section][aria-current=page]')).to_have_attribute('data-purchase-section','orders')
        expect(page.locator('#workspace-sidebar [data-purchase-section][aria-current=page]')).to_have_attribute('data-purchase-section','orders')
        expect(page.locator('[data-purchase-menu]')).to_have_attribute('open','')
        page.locator('[data-purchase-menu] > summary').focus()
        ...
        expect(page.locator('.purchase-tabs [data-purchase-section][aria-current=page]')).to_have_attribute('data-purchase-section','suppliers')
        ...
        expect(page.locator('.purchase-tabs')).to_be_visible()
        ...
        page.locator('#workspace-sidebar [data-purchase-section=suppliers]').click()
        expect(page.locator('#workspace-sidebar')).to_have_attribute('aria-hidden','true')
        expect(page.locator('.purchase-tabs [aria-current=page]')).to_have_attribute('data-purchase-section','suppliers')
        assert page.evaluate('document.documentElement.scrollWidth<=innerWidth+1')
        page.screenshot(path=str(artifacts/'navigation-b-mobile-page.png'),full_page=True)
        page.locator('.purchase-tabs [data-purchase-section=requests]').click()
        page.wait_for_url('**/manager/magazzino/richieste')
        active=page.locator('.purchase-tabs [aria-current=page]')
        expect(active).to_have_attribute('data-purchase-section','requests')
        assert active.evaluate('(el)=>{const p=el.parentElement;return el.offsetLeft>=p.scrollLeft-1 && el.offsetLeft+el.offsetWidth<=p.scrollLeft+p.clientWidth+1}')
        assert page.evaluate('document.documentElement.scrollWidth<=innerWidth+1')
