...
def import_pdf(data: bytes, page_number: int = 1):
    ...
    try:
        with pdfplumber.open(BytesIO(data)) as pdf:
            ...
            for label in labels:
                ...
                panels.append({'key':uuid4().hex,'label':re.sub(r'\s+','',label['text']),
                    'points':[[min(w,max(0,x)),min(h,max(0,y))] for x,y in points],
            ...
            typical=median(scales) if len(scales)>=5 else None
            for p in panels:
                if counts[p['label'].upper()]>1: p['warnings'].append('Sigla ripetuta: verificare il collegamento')
                if typical and p['width_m'] and abs(math.dist(p['points'][0],p['points'][1])/p['width_m']/typical-1)>.15:
                    p['recognition']='estimated'
                    p['warnings'].append('Sagoma e quota non coerenti con la scala prevalente: controllare gli estremi')
            pages=len(pdf.pages)
        ...
        return {'width':w,'height':h,'panels':panels,'page_count':pages,
