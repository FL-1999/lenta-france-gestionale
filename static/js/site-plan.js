...
(() => {
  ...
  async function load(id, draft = false) {
    try {
      ...
      plan = data.plan; editing = !!plan?.editing; dirty = false; addMode = false; shapeWidths.clear();
  ...
  function inBounds(points) { return points.every(([x,y]) => Number.isFinite(x)&&Number.isFinite(y)&&x>=0&&y>=0&&x<=plan.layout.width&&y<=plan.layout.height); }
  function updatePoints(p, points) { if (!inBounds(points)) return false; p.points = points; p.reviewed = false; markDirty(); return true; }
  function renderMaps() {
    if (!plan) return;
    ...
    plan.layout.panels.forEach(p => {
      ...
      groups.forEach((group,i) => {
        ...
        if (active && editing) p.points.forEach((v,j)=>group.append(svgEl('circle',{cx:v[0],cy:v[1],r:Math.max(4,box[2]/130),class:'sp-handle','data-key':p.key,'data-corner':j})));
      });
    });
  ...
  function renderDetail() {
    ...
    q('[data-warnings]').textContent = !p ? '' : editing ? (p.warnings || []).join(' · ') : '';
    if (p) {
      form.elements.label.value = p.label; form.elements.width_m.value = p.width_m ?? ''; form.elements.element.value = p.element ?? ''; form.elements.reviewed.checked = p.reviewed;
  ...
  form.addEventListener('change',e=>{
    ...
    if(name!=='reviewed')p.reviewed=false;markDirty();render();
  });
  q('[data-scale]').onclick = () => {const p=panel(),reference=shapeWidths.get(p?.key);if(!p?.width_m||!reference){message('Per questo pannello imposta prima la dimensione grafica dagli angoli o dai campi.',true);return;}const g=geometry(p);g.len*=p.width_m/reference;if(updatePoints(p,rectangle(g))){shapeWidths.set(p.key,p.width_m);render();}else message('Il ridimensionamento porta il pannello fuori dal foglio.',true);};
