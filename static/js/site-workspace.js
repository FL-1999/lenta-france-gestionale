...
(() => {
  const tr = value => window.LentaText(value);
  ...
  const number = v => v == null ? '—' : Number(v).toLocaleString(fr?'fr-FR':'it-IT',{maximumFractionDigits:3});
  ...
  async function api(path,method,body) {
    ...
    const result=await response.json().catch(()=>({detail:tr("Risposta non valida. Ricarica la pagina.")}));
    if(!response.ok)throw Error(typeof result.detail==='string'?result.detail:tr("Controlla i valori inseriti."));
    return result;
  }
  ...
  function renderGroups() {
    q('#sw-groups').innerHTML=groups.length?groups.map(g=>`<div class="sw-list-row"><div><strong>${esc(g.label)}</strong><div class="sw-muted">${g.kind==='angle'?tr("Angolo · fiche unica"):tr("Getto congiunto · fiches separate")}${g.total_m3==null?tr(" · da registrare"):` · ${number(g.total_m3)} m³`}</div></div><button type="button" class="btn btn-secondary" data-group="${g.id}">${canEdit?tr("Apri / modifica"):tr("Apri")}</button></div>`).join(''):`<p class="sw-muted">${tr("Seleziona i pannelli sulla pianta per creare un angolo o un getto congiunto.")}</p>`;
  }
  function openGroup(id) {
    const g=groups.find(g=>g.id===id);if(!g)return;
    editor(`<div class="sw-row"><h2>${esc(g.label)}</h2><button type="button" class="btn btn-secondary" data-close-editor>${tr("Chiudi")}</button></div><p>${g.kind==='angle'?tr("Una fiche per i due bracci. Larghezze nette confermate; il disegno della fiche mostra lo sviluppo complessivo."):tr("Compila le fiches dei singoli pannelli, poi registra una sola volta data e volume del getto.")}</p>${g.members.map(m=>`<div class="sw-list-row"><span>${esc(m.label)} · ${number(m.width)} × ${number(m.depth)} × ${number(m.thickness)} m</span><a class="btn btn-secondary" href="${m.fiche_id?`/manager/fiches/${m.fiche_id}`:`/manager/fiches/nuova?cantiere_id=${site}&numero_pannello=${m.number}&tipologia_scavo=paratia`}">${m.fiche_id?tr("Apri fiche"):tr("Crea fiche")}</a></div>`).join('')}${canEdit&&g.kind==='joint'?`<form id="sw-cast-form" data-group-id="${g.id}"><div class="sw-editor-grid"><label>${tr("Data getto")}<input type="date" name="cast_date" required value="${g.cast_date||''}"></label><label>${tr("Calcestruzzo totale · m³")}<input type="number" step="0.001" min="0.001" max="100000" name="total_m3" required value="${g.total_m3??''}"></label></div><label><input type="checkbox" name="manual"> ${tr("Modifica manualmente la ripartizione")}</label><div id="sw-allocation"></div><p class="sw-muted">${tr("Ripartizione stimata sui volumi teorici; a profondità e spessore uguali è proporzionale alle larghezze.")}</p><button type="submit" class="btn btn-primary">${tr("Conferma getto e quantità")}</button></form>`:''}${canEdit?`<div class="sw-list-row"><span class="sw-muted">${tr("Per cambiare pannelli: sciogli il gruppo e ricrealo.")}</span><button type="button" class="btn btn-danger" data-remove-group="${g.id}">${tr("Sciogli gruppo")}</button></div>`:''}`);
    const form=q('#sw-cast-form');if(form){form.addEventListener('input',()=>preview(g,form));preview(g,form);if(g.total_m3!=null&&g.members.every(m=>m.allocated_m3!=null)){const inputs=[...form.querySelectorAll('[data-allocation]')];form.elements.manual.checked=inputs.some((input,i)=>Math.abs(Number(input.value)-g.members[i].allocated_m3)>.00001);inputs.forEach((input,i)=>{input.value=g.members[i].allocated_m3;input.disabled=!form.elements.manual.checked;});}form.addEventListener('submit',async e=>{e.preventDefault();const total=Number(form.elements.total_m3.value),manual=form.elements.manual.checked?[...form.querySelectorAll('[data-allocation]')].map(i=>Number(i.value)):null;if(!confirm(tr("Confermare il totale e aggiornare le quantità delle fiches selezionate?")))return;const submit=form.querySelector('[type=submit]');submit.disabled=true;try{await api('/getti/'+g.id,'PUT',{revision:g.revision,total_m3:total,cast_date:form.elements.cast_date.value,manual,confirm:true});location.reload();}catch(err){message(err.message);submit.disabled=false;}});}
  }
  function preview(g,form){
    ...
    q('#sw-allocation').innerHTML=g.members.map((m,i)=>`<label class="sw-list-row"><span>${esc(m.label)} · ${number(weights[i]/sum*100)}%</span><input class="form-control" aria-label="${tr("Quantità")} ${esc(m.label)}" type="number" min="0" step="0.001" data-allocation value="${parts[i]/1000}" ${form.elements.manual.checked?'':'disabled'}> m³</label>`).join('');
  }
  ...
  root.addEventListener('click',async e=>{
    ...
    const create=e.target.closest('[data-make-group]');if(create){
      ...
      if(selected.size<2){message(tr("Seleziona almeno due pannelli, oppure un braccio A/B per individuare il suo compagno."));return;}
      const labels=[...selected].map(n=>elements.get(n)?.label||n).join(' + ');
      const prompt=kind==='angle'?(fr?`Créer l’angle ${labels} ? Confirmez-vous que les largeurs des deux branches sont nettes, sans compter deux fois l’intersection ?`:`Creare l’angolo ${labels}? Confermi che le larghezze dei due bracci sono nette, senza contare due volte l’intersezione?`):(fr?`Regrouper ${labels} dans un bétonnage groupé ? Les fiches restent séparées.`:`Raggruppare ${labels} in un getto congiunto? Le fiches rimangono separate.`);
      if(!confirm(prompt))return;create.disabled=true;
      try{const g=await api('/getti','POST',{numbers:[...selected],kind,confirm_net:kind==='angle'});groups.push(g);renderGroups();openGroup(g.id);message(tr("Gruppo salvato."));}catch(err){message(err.message);}finally{create.disabled=false;}return;
    }
    const remove=e.target.closest('[data-remove-group]');if(remove){const g=groups.find(g=>g.id===Number(remove.dataset.removeGroup));const hasFiche=g.members.some(m=>m.fiche_id);const warning=g.kind==='angle'&&hasFiche?tr("Verrà eliminata definitivamente anche la fiche unica dell’angolo. I pannelli rimarranno nella pianta."):tr("Le fiches rimangono; per il getto congiunto saranno ripristinate quantità e date precedenti alla ripartizione.");if(!confirm(tr("Sciogliere il gruppo? ")+warning))return;remove.disabled=true;try{await api(`/getti/${g.id}?revision=${g.revision}&confirm=true&delete_fiche=${g.kind==='angle'&&hasFiche}`,'DELETE');location.reload();}catch(err){message(err.message);remove.disabled=false;}}
  });
  draw();renderGroups();
})();
