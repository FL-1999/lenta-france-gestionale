...
Cartelle create automaticamente: `Gestionale/Cantieri/CODICE - NOME [CID]/Piante/Pianta 01`, `Documenti`, `Fiches`, `Dossier finali`, `Fatture`; fatture senza cantiere in `Gestionale/Acquisti/Fatture`. Esempio: `26-340 - NISSA CAMPUS [C3]`. Solo i PDF originali delle piante convalidate sono trasferiti. Bozze e anteprime restano nel database e nella sezione **Solo nel gestionale**, senza nuovo invio a SharePoint.

La prima convalida assegna il numero 1 per cantiere, poi 2, 3… Le prove non consumano numeri. La tabella `cloud_plan_publications`, creata all'avvio, conserva l'associazione anche dopo la rimozione degli originali. La convalida ripetuta dello stesso disegno non cambia il numero. Al primo avvio le piante attive già convalidate vengono numerate in ordine di prima convalida disponibile (data di caricamento come ripiego). I PDF mantengono il nome originale; gli altri documenti mantengono il suffisso ` - copia ID`. Gli hash restano nel database. Il percorso del cantiere viene riutilizzato dopo cambi del nome.

Il PDF è l'originale caricato: le modifiche alla geometria dei pannelli sono dati nel database, non riscrivono il PDF. Per recuperare anche queste modifiche serve il backup del database. Le modifiche dirette in SharePoint NON riscrivono i dati del gestionale.

### Riordino dei file già trasferiti

Con sincronizzazione attiva, dopo l'invio dei documenti il worker riordina fino a 3 copie verificate per ciclo che hanno ancora un nome con hash o un percorso `Piante/Disegno ID`. Usa il medesimo ID SharePoint per spostare e rinominare il file nella stessa raccolta, controllando contenuto prima e dopo, conflitti di destinazione ed eTag. Non ricarica né elimina file; lascia le vecchie cartelle vuote. Esclusi, cestino, copie eliminate, bozze e anteprime non partecipano. Le vecchie bozze e anteprime già su SharePoint mantengono ID e percorso remoto: nessuna cancellazione automatica. Per rimuoverle, il titolare le seleziona in **Solo nel gestionale**, le sposta nel cestino privato e conferma l’eliminazione definitiva. Le vecchie cartelle possono essere rimosse manualmente solo dopo aver verificato che siano vuote. Le copie senza più cantiere ricevono una cartella `Cantiere rimosso [CID]`.
...
Le copie mostrano nome e numero del cantiere, data di caricamento dell'originale (UTC), numero ufficiale della pianta convalidata (oppure Bozza) e collegamento per aprire esattamente quella pianta. PDF e anteprima condividono le etichette che identificano la pianta. **Pianta attualmente in uso** segue la scelta predefinita del cantiere: ultima pianta convalidata, oppure ultima bozza se non esistono piante convalidate. **Ultimo PDF caricato** identifica invece l'importazione più recente, anche se ancora in bozza o successivamente rimossa. Le due indicazioni possono quindi riguardare disegni diversi. Se cantiere/originale non esistono più, la pagina lo segnala; quando la data originale non è più disponibile mostra **Archiviato il**, senza dedurre una data di caricamento. Le medesime informazioni e un avviso per la pianta in uso compaiono nel riepilogo di eliminazione definitiva.
...
- **Recupera tra gli esclusi** rende nuovamente disponibile il file senza avviare trasferimenti. **Includi nel trasferimento** lo rimette in coda soltanto se è un documento trasferibile (per le piante: PDF convalidato), attiva solo se `SHAREPOINT_SYNC_ENABLED=true`. Il download recupera il file; non ricrea i dati applicativi.
