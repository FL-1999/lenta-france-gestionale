...
Gli angoli raggruppati sono presentati come un pannello unico `P3 A/B` nell’editor, nella pagina cantiere e nelle coupe. Il contorno è l’unione geometrica dei due bracci: vengono rimossi i bordi interni senza riempire il vuoto interno della L, deformare le misure o colmare uno spazio reale. Una coppia scollegata richiede ancora il raccordo. I bracci conservano le rispettive identità per le fiche e per la separazione; il selettore «Braccio da regolare» permette di modificarli individualmente. Selezionare un angolo nella coupe assegna entrambi i bracci; rimuoverlo dall’editor rimuove la coppia in bozza, con annullamento disponibile. Le verifiche server su sovrapposizioni, misure nette e dati di produzione restano applicate alla convalida.

Le bozze PDF possono essere rimosse senza eliminare il cantiere. `Sostituisci PDF` analizza prima il nuovo documento e rimuove la bozza precedente nella stessa transazione solo dopo un’importazione valida; errori di lettura o conflitti di revisione lasciano il PDF precedente disponibile. Un disegno già convalidato non è rimovibile con questa azione: si importa una nuova versione in bozza, mantenendo l’originale e i collegamenti produttivi. La rimozione usa `site_plans.removed_at` (colonna nullable aggiunta dal bootstrap), conserva PDF, anteprima e ultima bozza salvata; l’elenco dei PDF rimossi e il ripristino sono riservati agli amministratori. Nessuna cancellazione definitiva è esposta. Rimozione e ripristino sono registrati nell’audit, con controllo cantiere, origine e revisione concorrente. La rimozione di una singola sagoma usa una conferma interna e può essere annullata prima del salvataggio, anche quando era l’ultimo pannello.
